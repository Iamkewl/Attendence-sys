...
### FR-11: Post-Enrollment Verification (new)

| ID | Requirement | Priority |
|----|-------------|----------|
| FR-11.1 | Provide a probe-image verification endpoint for enrolled students | P0 |
| FR-11.2 | Return ranked candidate identities with confidence scores and margin diagnostics | P0 |
| FR-11.3 | Include quality and detection metadata in verification response for troubleshooting | P0 |
| FR-11.4 | Log verification actions to audit trail with actor attribution | P1 |

### FR-12: Enrollment Diagnostics UX (new)

| ID | Requirement | Priority |
|----|-------------|----------|
| FR-12.1 | Enrollment modal must expose immediate feedback after guided burst capture | P0 |
| FR-12.2 | Enrollment modal must display progress toward template and pose coverage requirements | P0 |
| FR-12.3 | Enrollment modal must provide a post-enrollment test panel for probe-image verification | P0 |
...
| Method | Path | Auth | Role |
|--------|------|------|------|
| POST | `/students` | JWT | admin, instructor |
| GET | `/students` | JWT | admin, instructor |
| GET | `/students/{id}` | JWT | admin, instructor, self |
| PATCH | `/students/{id}` | JWT | admin, instructor |
| DELETE | `/students/{id}` | JWT | admin, instructor |
| POST | `/students/{id}/enroll/images` | JWT | admin, instructor |
| GET | `/students/{id}/enrollment/quality` | JWT | admin, instructor |
| GET | `/students/{id}/enrollment/templates` | JWT | admin, instructor |
| PATCH | `/students/{id}/enrollment/templates/{embedding_id}` | JWT | admin, instructor |
| GET | `/students/{id}/enrollment/analytics` | JWT | admin, instructor |
| GET | `/students/{id}/enrollment/analytics/history` | JWT | admin, instructor |
| POST | `/students/{id}/enrollment/test` | JWT | admin, instructor |
