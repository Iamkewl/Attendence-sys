        ...
        try:
            ...
            async for message in pubsub.listen():
                ...
                try:
                    ...
                    if message.get("channel") == CHANNEL_SYSTEM_ALERTS.encode():
                        try:
                            from backend.api.v1.sse import system_sse_broadcaster

                            await system_sse_broadcaster.publish(event)
                        except Exception as exc:
                            logger.debug("event_bridge: system SSE publish failed: %s", exc)
                        continue
                    ...
                    try:
                        ...
                        logger.debug("event_bridge: schedule SSE publish failed: %s", exc)
                    ...
                    try:
                        ...
                        logger.debug("event_bridge: WebSocket broadcast failed: %s", exc)
