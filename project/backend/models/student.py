...
class StudentEmbedding(Base):
    ...
    Each student can have embeddings for:
      ...
      - resolution: full | low_res
            - model_name: arcface | adaface | lvface
    ...
    capture_quality_score: Mapped[float | None] = mapped_column(
        Float, nullable=True
    )
    sharpness: Mapped[float | None] = mapped_column(Float, nullable=True)
    face_size_px: Mapped[int | None] = mapped_column(Integer, nullable=True)
    ...
    collision_risk: Mapped[float | None] = mapped_column(Float, nullable=True)
