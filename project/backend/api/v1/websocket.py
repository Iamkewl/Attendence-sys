...
from backend.api.deps import get_current_user_from_token
from backend.core.constants import UserRole
from backend.db.session import get_async_session
from backend.models.room import Device
from backend.services.hmac_auth import verify_device_secret
...
async def device_ws(
    ...
    - Server → Device: JSON {"action": "FLASH_CAPTURE", "flash_sequence": ["flash_off", "flash_on"], ...}
    - Device → Server: Binary image data or JSON status updates
    """
    result = await db.execute(
        select(Device).where(Device.id == device_id, Device.room_id == room_id)
    )
    device = result.scalar_one_or_none()
    if not device or not verify_device_secret(secret, device.secret_key_hash):
        await websocket.close(code=1008, reason="Invalid device credentials")
        return
