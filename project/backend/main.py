...
async def lifespan(app: FastAPI):
    ...
    try:
        from backend.core.config import (
            build_onnx_execution_providers,
            get_settings,
        )
        ...
        logger.info(
            "startup: AI pipeline models loaded (onnx_provider_chain=%s)",
            build_onnx_execution_providers(settings.insightface_provider),
        )
    ...
    try:
        ...
        logger.debug("shutdown: scheduler stop failed: %s", exc)
